...
wine_data["fixed acidity"] = np.log(wine_data["fixed acidity"] - min(wine_data["fixed acidity"])+1e-10)
#wine_data = wine_data.drop(['sulphates', 'free sulfur dioxide', 'pH'], axis=1)
wine_data = scale(wine_data)
nonlin_wine_data = np.hstack(
    (
        np.ones((len(wine_data), 1)),
        #np.log(wine_data[:, 0] - min(wine_data[:, 0])+1e-10).reshape(len(wine_data), 1),
        #(wine_data[:,0]*wine_data[:,-2]).reshape(len(wine_data), 1),
        wine_data,
    )
)
