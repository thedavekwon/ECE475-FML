...
train = prostate_data[prostate_data["train"] == "T"].drop(["train"]).to_numpy()
train = np.hstack((np.ones((len(train), 1)), train))
test = prostate_data[prostate_data["train"] == "F"].drop(["train"]).to_numpy()
...
sigma_hat_squared = np.sum((y_train - y_pred) ** 2) / (X_train.shape[0] - X_train.shape[1] - 2)
std_error = np.sqrt(sigma_hat_squared * np.diag(inv(X_train.transpose() @ X_train)))
