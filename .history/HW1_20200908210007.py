...
print(std_error)
print(z_score)
pd.DataFrame({"Coefficient": prostate_LR.coef_(), "std error": std_error, "Z Score": z_score}, index=["intercept"]+column_names)
...
for l in ls:
    ...
    mses.append(((y_pred - val[:, -1]) ** 2).mean())
plt.plot(ts, np.array(beta_lasso))
