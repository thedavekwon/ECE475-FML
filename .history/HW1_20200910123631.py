...
##### Nonlinear 
# lol sure we can just do whatever
# but thats what lasso implicitly does right? removes features ye but all good he just said any kind of non-lienar interaction
# just messing around rn
nonlin_wine_data = np.hstack((np.square(wine_data[:,0]*wine_data[:,7]).reshape(len(wine_data),1), wine_data))
new_columns = ["fixed acidity^2"]
