...
def stepwiseLogisticRegression(X_train, y_train, X_test, y_test):
    ...
    while len(idxs) != len(removed_feature):
        for idx in idxs:
            if idx in removed_feature:
                continue
            LR = LogisticRegression(0.01, 16)
            X_train_temp = X_train[:, feature_order+[idx]].reshape(X_train.shape[0], len(feature_order)+1)
            train_accuracies = []
            ...
            print(len(column_names), len(train_accuracies), idx)
            print(f"{column_names[idx]} Train Accuracy: {train_accuracies[idx]}")
