        ...
        for i in range(X.shape[0]):
            ...
            for j in range(X.shape[1]):
                if self.c and self.regularization == "L2":
                    self.theta[j] = self.theta[j] + self.step * (self.c*(y[i] - sigmoid(self.theta.T @ X[i, :])) * X[i][j] + temp_theta_sum)
                else:
                    self.theta[j] = (
                        self.theta[j]
...
def logisticRegressionL2(X_train, y_train, X_test, y_test):
    LR = LogisticRegression(0.01, 16, 10, "L2")
