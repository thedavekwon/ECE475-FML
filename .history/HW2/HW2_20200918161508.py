...
def stepwiseLogisticRegression(X_train, y_train, X_test, y_test):
    ...
    # features = [x in range(X_train.shape()[1])]
    idxs = list(range(X_train.shape[1]))
    # Training our logistic regression model on the training set

    while len(idxs) != len(removed_feature):
        train_accuracies = []
        left_features = list(set(idxs)-set(removed_feature))
        for idx in range(len(left_features)):
            if left_features[idx] in removed_feature:
                continue
            LR = LogisticRegression(0.01, 16)
            X_train_temp = X_train[:, feature_order+[left_features[idx]]].reshape(X_train.shape[0], len(feature_order)+1)
            ...
            print(f"{column_names[left_features[idx]]} Train Accuracy: {train_accuracies[left_features[idx]]}")
        
        train_accuracies = np.array(train_accuracies)    
        if not feature_train_accuracies or any(train_accuracies > feature_train_accuracies[-1]):
            selected_idx = np.argmax(train_accuracies)
            feature_order.append(selected_idx)
            feature_train_accuracies.append(train_accuracies[selected_idx])
            print(idxs, selected_idx)
            removed_feature.append(selected_idx)
        else:
            break
    print(feature_order)
