...
def stepwiseLogisticRegression(X_train, y_train, X_test, y_test):
    ...
    while len(idxs) != len(removed_feature):
        train_accuracies = []
        left_features =  set(idxs)-set(removed_feature)
        for idx in set(idxs)-set(removed_feature):
            if idxs[idx] in removed_feature:
                continue
            LR = LogisticRegression(0.01, 16)
            X_train_temp = X_train[:, feature_order+[idxs[idx]]].reshape(X_train.shape[0], len(feature_order)+1)
            ...
            print(f"{column_names[idxs[idx]]} Train Accuracy: {train_accuracies[idxs[idx]]}")
