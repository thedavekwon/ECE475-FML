...
class LogisticRegression:
    def __init__(self, step, batch_size, c=None, regularization=None):
        self.theta = None
        self.step = step
    ...
    def fit(self, X, y):
        ...
        for i in range(X.shape[0]):
            ...
            temp_theta_sum = np.sum(self.theta)
            print(temp_theta_sum)
            for j in range(X.shape[1]):
                if self.c and self.regularization == "L2":
                    self.theta[j] = self.theta[j] + self.step * (
                        self.c * (y[i] - sigmoid(self.theta.T @ X[i, :])) * X[i][j]
                        + temp_theta_sum
                    )
                else:
                    self.theta[j] = (
...
def logisticRegressionL2(X_train, y_train, X_test, y_test):
    LR = LogisticRegression(0.01, 16, 0.1, "L2")
...
def stepwiseLogisticRegression(X_train, y_train, X_test, y_test):
    ...
    while len(idxs) != len(selected_features):
        ...
        if not feature_train_accuracies or any(
            ...
            break
    print(f"Selected feature in order: {list(map(lambda x: column_names[x], selected_features))}")
    print(f"Accurarcies adding featres: {feature_train_accuracies}")
...
# logisticRegressionL2(X_train, y_train, X_test, y_test)
