...
# Q2                                                                             #
##################################################################################
def ridge_regression(X_train, y_train, test, val, column_names):
    mses = []
    beta_ridges = []
    for l in ls:
    ...
    y_pred = ridge(test[:, 1:-1])
    mse = ((y_pred - test[:, -1]) ** 2).mean()
    print(f"Lambda: {ls[idx]}")
    print(f"MSE: {mse}")


# ridge_regression(X_train, y_train, test, val, column_names)

##################################################################################
# Q3                                                                             #
##################################################################################
def lasso_regression(X_train, y_train, test, val, column_names):
    mses = []
    beta_lasso = []
...
# lasso_regression(X_train, y_train, test, val, column_names)
...
##### Nonlinear
# wym just pca principle componenet analysis (Basically just use the highest n principle compnenet)
# like just add it as a term? I'm not sure if that'll work
nonlin_wine_data = np.hstack((wine_data[:,0]*wine_data[:,6], wine_data[:,0]*wine_data[:,7], wine_data))
column_names.extend(["fixed acidity x density", "fixed acidity x pH"])
train, temp = train_test_split(wine_data, test_size=0.2)
test, val = train_test_split(temp, test_size=0.5)
...
# print(column_names)
col = 3
plt.plot(X_train[:, col], y_train, "x")

# plt.xlabel("")
# plt.ylabel("")
# plt.title("Linear Regression")
plt.legend(column_names[col])
plt.show()
