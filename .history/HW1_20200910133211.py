...
prostate_df.corr()  # Find correlation between features
...
pca.fit(X_train)
X_train = np.hstack((np.ones((len(wine_data), 1)), pca.transform(X_train)))
X_test = np.hstack((np.ones((len(wine_data), 1)), pca.transform(X_train)))
X_val = np.hstack((np.ones((len(wine_data), 1)), pca.transform(X_train)))
